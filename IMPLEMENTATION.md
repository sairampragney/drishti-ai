...
Predefined accounts are `sathwik@gmail.com` (Dr. Sathwik), `madhurima@gmail.com` (Dr. Madhurima), and `sairampragney@gmail.com` (Dr. Sairam Pragney). Any password of at least six characters is accepted for these demo accounts. Signup creates a browser-local account.
...
The scanner uses `@mediapipe/tasks-vision` locally in the browser. It requests the camera, detects a face, estimates both eye landmark groups, draws two black boxes, and enables results only after detection. It does not diagnose diabetic retinopathy.
